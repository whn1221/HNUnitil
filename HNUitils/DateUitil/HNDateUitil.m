...
#import "HNDateUitilDefine.h"

@implementation HNDateUitil
...
{
    static NSDateFormatter *_HNDateFormatter = nil;
    static dispatch_once_t onceToken;
    
    HNDispatchOnce(onceToken, ^{
        
        if (!_HNDateFormatter) {
            _HNDateFormatter = [[NSDateFormatter alloc]init];
        }
    });
    return _HNDateFormatter;
}

+ (NSString *)detailTimeAgoString:(NSDate *)date
{
    if (HNCheckObjectNull(date)) {
        return nil;
    }
    long long timeNow = [date timeIntervalSince1970];
    NSCalendar * calendar=[HNDateUitil sharedCalendar];
...
{
    return [HNDateUitil detailTimeAgoString:[HNDateUitil dateFromTimeInterval:timeInterval]];
}
...
+ (NSString *)stringDaysAgoAgainstMidnight:(BOOL)flag withDate:(NSDate *)date {
    ...
    NSUInteger daysAgo = (flag) ? [HNDateUitil daysAgoAgainstMidnight:date] : [HNDateUitil daysAgoFromNow:date];
    NSString *text = nil;
    switch (daysAgo) {
        case 0:
            text = @"今天";
...
+ (NSString *)stringDaysAgo:(NSDate *)date
{
    ...
    return [HNDateUitil stringDaysAgoAgainstMidnight:YES withDate:date];
}
...
+ (NSString *)weekDayString:(NSDate *)date
{
    ...
    NSString *weekName = [weekNameDict objectForKey:@(HNDateGetWeekDay(date))];
...
+ (NSString *)weekNumberString:(NSDate *)date
{
    ...
    return [NSString stringWithFormat:@"第%lu周",(unsigned long)HNDateGetWeekNum(date)];
}
...
{
    if (HNStringIsNull(string)) {
        return nil;
    }
    return [HNDateUitil dateFromString:string withFormat:kNSDateHelperFormatSQLDate];
}
...
+ (NSDate *)dateTimeFromString:(NSString *)string
{
    ...
    return [HNDateUitil dateFromString:string withFormat:kNSDateHelperFormatSQLDateWithTime];
}

+ (NSDate *)dateFromString:(NSString *)string withFormat:(NSString *)format
{
    if (HNCheckKeyValueHasNull(string, format)) {
        return nil;
    }
...
{
    if (HNCheckKeyValueHasNull(string, date)) {
        return nil;
    }
...
+ (NSString *)stringFromDate:(NSDate *)date
{
    ...
    return [HNDateUitil stringFromDate:date withFormat:kNSDateHelperFormatSQLDateWithTime];
}
...
{
    return [HNDateUitil birthdayToAge:HNDateFromTimeInterval(date)];
}

+ (NSString *)dateToConstellation:(NSDate *)date
{
    NSInteger day = [HNDateUitil day:date];
    NSInteger month = [HNDateUitil month:date];
...
{
    return [HNDateUitil dateToConstellation:HNDateFromTimeInterval(date)];
}
